package com.example.myapplicationtest1.game.preset.bullet;
...
import com.example.myapplicationtest1.game.physics.DstCntDynam;
...
public class Bullet extends GHQObject implements HasPoint, HasDotPaint {
	...
	public void init() {
		...
		limitFrame = Integer.MAX_VALUE;
		limitRange = Integer.MAX_VALUE;
		penetration = 0;
		reflection = 0;
		accel = 0.0;
	...
	public boolean checkIsOutOfRange(double range) {
		return range != Integer.MAX_VALUE && range <= ((DstCntDynam)point()).getMovedDistance();
	}
	...
	public final boolean dynamIdle() {
		return dynamIdle(Integer.MAX_VALUE);
	}
	...
	public boolean hitObjectDeleteCheck(GHQObject object) {
		...
		hitObject(object);
		if(penetration > 0) {
			if(penetration != Integer.MAX_VALUE)
				penetration--;
		}else {
	...
	public void split_Round(int radius,int amount) {
		final double D_ANGLE = 2*Math.PI/amount;
