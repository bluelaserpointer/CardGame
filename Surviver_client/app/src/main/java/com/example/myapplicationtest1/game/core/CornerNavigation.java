...
import com.example.myapplicationtest1.game.physics.Route;
...
public class CornerNavigation {
	...
	protected class Corner extends Point.IntPoint{
		...
		void explore0() {
			...
			Point neighbors[] = Direction4.getVertHorzClosest_int(this, corners, Direction4.S, Direction4.D, Direction4.S, Direction4.A);
		...
		Anchor[] explore0(Anchor settedAnchors[]) {
			...
			Point neighbors[] = Direction4.getVertHorzClosest_int(this, corners, Direction4.W, Direction4.D, Direction4.S, Direction4.A);
			...
			if(hasWall(point.shift(direction, EQUAL_GAP))) { //has wall in first step
				...
				while(true) {
					if(neighborPoint != null && !point.checkDirection(neighborPoint, direction)) { //arrive at the neighbor corner point
						...
						DebugEffect.setLine(Color.BLUE, 3F, intX(), intY(), neighborPoint.intX(), neighborPoint.intY());
						return null;
					}
	...
	public void defaultCornerCollect() {
		...
		for(;point.intX() < X_LIMIT;point.shift(Direction4.D, EQUAL_GAP)) {
			point.setY(EQUAL_GAP/2);
			oldBlockState = hasWall(point);
			while(point.intY() < Y_LIMIT) {
				point.shift(Direction4.S, EQUAL_GAP);
