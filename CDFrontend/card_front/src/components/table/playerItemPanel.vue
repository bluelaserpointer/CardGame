  ...
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="panelVisible" top="5vh" class="editDialog">
      <el-form ref="temp" :rules="rules" :model="temp" style="margin: auto 50px auto 50px; display:grid; grid-template-columns: 50% 50%; grid-column-gap: 10px" class="demo-form-inline">
        <el-form-item v-if="dialogStatus==='update' " label="ID" prop="ownItemId">
          <el-input v-model="temp.ownItemId" disabled />
        </el-form-item>
        <el-form-item v-if="dialogStatus==='create'" label="UserId" prop="userId">
          <el-input v-model="temp.userId" class="userIdOwnItemInput" />
        </el-form-item>
        <el-form-item v-if="dialogStatus==='create'" label="ItemId" prop="itemId">
          <el-input v-model="temp.itemId" class="itemIdOwnItemInput" />
        </el-form-item>
        <el-form-item v-else label="UserId" prop="userId">
          <el-input v-model="temp.userId" class="userIdOwnItemInput" disabled />
        </el-form-item>
        <el-form-item v-else label="ItemId" prop="itemId">
          <el-input v-model="temp.itemId" class="itemIdOwnItemInput" disabled />
        </el-form-item>
        <el-form-item label="ItemCount" prop="itemCount">
          <el-input v-model="temp.itemCount" class="itemCountOwnItemInput" />
        </el-form-item>
        <el-form-item v-if="dialogStatus==='update'" label-width="120px" label="AccquireDate" class="postInfo-container-item">
          <el-date-picker v-model="temp.accquireDate" class="dateOwnItemInput" type="datetime" value-format="yyyy-MM-dd hh:mm:ss" placeholder="Select date and time" />
        </el-form-item>
      </el-form>
      ...
      <div slot="footer" class="dialog-footer outerDialog">
        <el-dialog
          ...
          class="innerDialog"
        >
          <el-input v-model="confirmPassword" class="confirmOwnItemInput" placeholder="Identification" show-password width="60%" />
          ...
          <span slot="footer" class="dialog-footer">
            ...
            <el-button v-if="confirmDelete === false" class="deleteOwnItemInnerButton" type="danger" disabled>Delete</el-button>
            <el-button v-else class="deleteOwnItemInnerButton" type="danger" @click="deleteData">Delete</el-button>
          </span>
        </el-dialog>
  ...
  methods: {
    ...
    submitUpdate() {
      ...
      request.post('ownItem/updateOwnItem', JSON.stringify(postData)).then(response => {
        if (response.data) {
          // TODO: SHORTEN THE REQUESTS
          _this.getList(_this.listQuery.page, _this.listQuery.limit)
          _this.panelVisible = false
          _this.resetTemp()
        } else {
          _this.$message.error('Updating Data failed!')
        }
      })
        .catch(error => {
          _this.$message.error('Updating Data failed!')
